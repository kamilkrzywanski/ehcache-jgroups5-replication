...
public class JGroupsCacheReceiver implements Receiver {
    ...
    public void receive(Message msg) {
        if (msg == null || msg.getLength() == 0) {
            LOG.warn("Received an empty or null Message: {}", msg);
            return;
        }

        final Object object;
        try {
            object = Util.objectFromByteBuffer(msg.getObject());
        } catch (IOException | ClassNotFoundException e) {
            throw new RuntimeException(e);
        }
        if (object == null) {
            LOG.warn("Received a Message with a null object: {}", msg);
            return;
        }
        ...
        if (object instanceof JGroupEventMessage) {
            ...
            for (final Object message : messages) {
                ...
                if (message instanceof JGroupEventMessage) {
                    ...
                    LOG.warn("Received message of type {} but member was of type '{}' and not {}. Member ignored: {}", List.class, message.getClass(), JGroupEventMessage.class, message);
                }
            }
        } else {
            LOG.warn("Received message with payload of type {} and not {} or List<{}>. Message: {} payload {}", object.getClass(), JGroupEventMessage.class, JGroupEventMessage.class.getSimpleName(), msg, object);
        }
    }
    ...
    private void handleEhcacheNotification(final JGroupEventMessage message, final String cacheName) {
        ...
        switch (message.getEvent()) {
            ...
            default: {
                LOG.warn("Unknown JGroupsEventMessage type received, ignoring message: {}", message);
                break;
            }
        }
    }
