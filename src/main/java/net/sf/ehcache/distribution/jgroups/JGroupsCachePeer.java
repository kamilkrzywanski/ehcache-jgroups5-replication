...
public class JGroupsCachePeer implements CachePeer {
    ...
    public List<Address> getOtherGroupMembers() {
        ...
        final List<Address> addresses = new ArrayList<>(members.size() - 1);
    ...
    public void send(Address dest, List<JGroupEventMessage> eventMessages) {
        ...
        final LinkedList<JGroupEventMessage> synchronousEventMessages = new LinkedList<>();
        ...
        if (synchronousEventMessages.isEmpty()) {
            return;
        }
    ...
    private Queue<JGroupEventMessage> getMessageQueue(long asyncTime) {
        ...
        if (queue == null) {
            final Queue<JGroupEventMessage> newQueue = new ConcurrentLinkedQueue<>();
    ...
    private void sendData(Address dest, List<? extends Serializable> dataList) {
        ...
        final Message msg = new ObjectMessage(dest, data);
        try {
    ...
    private void flushQueue(Queue<JGroupEventMessage> queue) {
        final ArrayList<JGroupEventMessage> events = new ArrayList<>(CHUNK_SIZE);
