...
public class JGroupsBootstrapManager {
    ...
    private final class BootstrapRequestRunnable extends ThreadNamingRunnable {
        ...
        public void runInternal() {
            ...
            try {
                ...
                LOG.debug("Loading cache {} with local address {} from peers: {}", cacheName, localAddress, addresses);
                ...
                do {
                    ...
                    var addressBytes = new ByteArrayOutputStream();
                    try {
                        Util.writeAddress(localAddress, new DataOutputStream(addressBytes));
                    } catch (IOException e) {
                        throw new RuntimeException(e);
                    }
                    JGroupEventMessage event = new JGroupEventMessage(JGroupEventMessage.BOOTSTRAP_REQUEST, addressBytes.toByteArray(), null, cacheName);
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Requesting bootstrap of {} from {}", cacheName, address);
                    }

                    cachePeer.send(address, List.of(event));
                ...
                } while (bootstrapRequest.getBootstrapStatus() != BootstrapStatus.COMPLETE && !addresses.isEmpty());
                ...
                if (BootstrapStatus.COMPLETE == bootstrapRequest.getBootstrapStatus()) {
                    ...
                    LOG.info("Bootstrap for cache {} ended with status {}, loaded {} elements",
                            cacheName, bootstrapRequest.getBootstrapStatus(), replicationCount);
                }
            } finally {
        ...
        protected void waitForBootstrap(final String cacheName, final Address address) {
            ...
            LOG.warn("Bootstrap of {} did not complete in {}ms, giving up on bootstrap request to {}.",
                    cacheName, BOOTSTRAP_RESPONSE_MAX_TIMEOUT, address);
        }
    ...
    private final class BootstrapResponseRunnable extends ThreadNamingRunnable {
        ...
        @Override
        public void runInternal() {
            var bytes = new ByteArrayInputStream((byte[]) this.message.getSerializableKey());
            final Address requestAddress;
            try {
                requestAddress = Util.readAddress(new DataInputStream(bytes));
            ...
            if (cache == null) {
                ...
                final JGroupEventMessage bootstrapCompleteMessage = new JGroupEventMessage(JGroupEventMessage.BOOTSTRAP_INCOMPLETE,
                        null, null, cacheName);
                cachePeer.send(requestAddress, List.of(bootstrapCompleteMessage));
        ...
        private void sendResponseChunk(Ehcache cache, Address requestAddress, List<JGroupEventMessage> events) {
            LOG.debug("reply {} elements to {} to bootstrap cache {}", events.size(), requestAddress, cache.getName());
