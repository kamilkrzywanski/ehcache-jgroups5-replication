...
public class JGroupsReplicationTest {
    ...
    public void setUp(TestInfo testInfo) {
        CacheTestUtilities.startTest(testInfo.getTestMethod().map(Method::getName).orElse("unknown"));
        LOG.info("SETUP");
    ...
    @Test
    public void testCASOperationsNotSupported() {
        LOG.info("START TEST");
    ...
    public void testSimultaneousPutRemove() throws InterruptedException {
        ...
        cache2.remove(element.getObjectKey());
        
        //Wait up to 2 seconds for the caches to become coherent
        CacheTestUtilities.waitForReplication(0, MAX_WAIT_TIME, cache1);

        assertNull(cache1.get(element.getObjectKey()));
        manager1.clearAll();
        ...
        assertNotNull(cache2.get(element.getObjectKey()));
